...
; Takes the first character of a string, makes it uppercase, and returns it
; with a space and the number of letters in the word.

(define (bingo-word word)
  (text (string-append (string-upcase(substring word 0 1)) " "
                       (number->string(string-length word))) 30 "lightblue"))
...
; Image –> Image
; Creates the image of a house.

(define HOUSE
  (above
   (triangle 80 "solid" "gray")
   (overlay/align "center" "bottom" (rectangle 15 30 "solid" "tan")
...
; Image –> Image
; Creates an image of a window.
(define WINDOW
  (square 15 "solid" "lightblue"))

; Image –> Image
; Overlays the windows onto the house.
(define HOUSE-WITH-WINDOWS
  (overlay/offset (overlay/offset WINDOW 30 0 WINDOW) 0 -20 HOUSE))
...
(define (scene t)
  (overlay/align "center" "bottom" HOUSE-WITH-WINDOWS (rectangle 500 200 "solid" (sky-color t))))
